import type { ReaderChromeSlots, ReaderSlotCtx } from '@react-epub-reader/reader'

/** 随感入口 — 点击后通过 bridge navigate 事件抛给 App */
export function createThoughtsMenuSlot(onNavigate: (path: string) => void): ReaderChromeSlots {
  return {
    topBarMoreMenu: (ctx: ReaderSlotCtx) => (
      <button
        type="button"
        className="reader-top-bar__menu-item"
        onClick={() => onNavigate(`/book/${ctx.bookId}/thoughts`)}
      >
        <svg
